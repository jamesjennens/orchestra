...
When the flag is set, `brief` also returns a bounded `newer` summary: counts split between the owner's own entries and other actors' entries, clipped other-actor names, up to five oldest unincorporated entry references with stable entry IDs, and a direct `history TASK` read path. In that state the checkpoint's recorded `next_action` is prefixed with a `STALE CHECKPOINT:` qualification instead of being presented as unconditional. Counts cover the full snapshot, including late or edited entries the five-entry excerpt omits; page `history` for complete coverage. Reading a briefing clears nothing: acknowledging a direction, reconciling it in a new checkpoint and completing it remain separate, explicit acts. The work queue mirrors the counts as `newer_activity_by_others` / `newer_activity_own` (`null` without a checkpoint or when checkpoint history is malformed), so `work --mine` and session resume surface directions that arrived after the owner's checkpoint.
