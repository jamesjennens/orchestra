...
Provenance is server-bound and round-trips: do not hand-craft incorporated digests. Omit them and the server computes and binds the exact snapshot provenance, or fetch the supported map with `checkpoint TASK --provenance` (returns the current `activity_cursor` and `provenance`) and embed the returned `provenance` verbatim — a missing, extra or altered map is rejected, so fabricated maps cannot be asserted. Each stored record keeps an exact digest window, a bounded truncated-digest map for older entries and a chain hash, and the server shrinks the window as needed so the final serialized record always fits the 80 KB cap; the same cap is enforced on read, so an accepted write always reads back.

Coverage is retained across the checkpoint chain, not in any single record: `checkpoint TASK --verify` classifies each entry against the evidence any checkpoint in the task's history recorded (exact window digests, then truncated older-map digests), and reports `coverage: verified` only when every current entry is covered. Entries no checkpoint ever recorded are reported `unverified` with `coverage: bounded` — they are never called fresh or unchanged — while a task whose checkpoints carry no per-entry evidence at all reports `coverage: unknown` and `verified_entries: 0`. There is no shortcut that verifies never-recorded entries: publishing more checkpoints records evidence for future verification, and never-recorded history must be reconciled manually via `history`/`show`. `brief` reports the same distinction (`unverified_count`, `coverage: windowed`).

`directions` optionally records explicit dispositions for other actors' comment directions: `{"id": "<task>-c<comment_id>", "state": "acknowledged|resolved|superseded", "digest": "<entry digest from --provenance>", "note": ..., "evidence": ...}`. Acknowledged means seen and taken into account — it is not completion. Resolved/superseded require note and evidence, and the digest must match the incorporated entry, so an edited direction re-surfaces until re-dispositioned. Dispositions are carried forward into later checkpoints automatically:
an ordinary checkpoint that omits `directions` keeps the existing dispositions, and an edit to the direction comment invalidates them via digest mismatch. A direction absent from `directions` stays outstanding on `brief` (and on `work`/`resume`, as `unresolved_directions`) even when checkpoint activity is current — receipt and cursor advancement are never resolution. Submit:
