...
Provenance is server-bound and round-trips: do not hand-craft incorporated digests. Omit them and the server computes and binds the exact snapshot provenance, or fetch the supported map with `checkpoint TASK --provenance` (returns the current `activity_cursor` and `provenance`) and embed the returned `provenance` verbatim — a missing, extra or altered map is rejected, so fabricated maps cannot be asserted. The stored record keeps an exact digest window (200), a bounded truncated-digest map for older entries (300) and a rolling chain hash over everything outside the window, and the server shrinks the window as needed so the final serialized record always fits the 80 KB cap; the same cap is enforced on read, so an accepted write always reads back. Entries beyond both recorded bounds are reported as `unverified` rather than as fresh, and `checkpoint TASK --verify` gives an exact server-side classification of the current snapshot against the stored checkpoint (including older edits and backdated arrivals). An identical retry reconciles on the already-stored record — provenance is normalized out of the request identity — so a lost response never causes a second write. Legacy payloads without provenance still parse for reads.
