...
1. Every protected request has one server-derived authenticated principal or is rejected. Login accepts anonymous credentials under throttling; reset redemption authorizes only the account bound to a valid single-use reset token. Neither grants access to project data before authentication.
...
single-use random reset value returned once to the authenticated superuser over TLS with no-store headers, then delivered through the operator's approved out-of-band channel, expires it after 30 minutes, stores only its hash, and
...
| Create a project (global account policy) | yes | authenticated creation policy | authenticated creation policy | authenticated creation policy |
...
| Issue worker credentials | all | own project | no | no |
| Revoke worker credentials | all | own project | own credential only | own credential only |
...
call; it does not authorize direct database access. Protected mutation rows require the authenticated principal, applicable current membership check, and a request ID. Login is anonymous; reset redemption uses the validated token ID and bound account as its limited authority. These exceptions cannot perform other account or project mutations.
...
| `POST /v1/accounts/{id}/password`, `POST /v1/accounts/{id}/reset`, `POST /v1/accounts/{id}/reset/redeem`, `POST /v1/accounts/{id}/disable` | Password change: account owner or superuser; reset issue/disable: superuser; reset redeem: reset recipient | Change requires current password or superuser; issue creates a pending single-use reset; redeem requires unexpired reset value and new password | Change key scoped to principal + account + operation; redeem key scoped to verified reset token ID + account + route; reset issue key scoped to admin + account; `204/200`, `401/403/409`; reset issue returns the value once to the authenticated superuser; redact logs and use no-store. A lost response retries to metadata only; explicitly invalidate the pending token and reissue with a new key. Reset redemption is scoped to token ID + account + route, with request hash checked separately | Password verifier update; reset-token consume; account disable and session/token revocation |
...
| `POST /v1/projects/{id}/tasks/{task}/reviews` | Contributor submits; project owner requests or approves | Current contribution and review chain are read under same authorization check | Key scoped to principal + project + task + route; `201`, `403/409` | Structured contribution/review protocol |
| `POST /v1/projects/{id}/tasks/{task}/checkpoints` | Contributor/owner; viewer denied | Checkpoint previous/cursor is current; task membership remains valid | Key scoped to principal + project + task + route; `201`, `403/409` | Canonical checkpoint append |
...
Every route uses the section 7 idempotency rule: payload fields, contribution IDs, previous versions, claim context and other preconditions are part of the canonical request hash, not extra key namespaces. For example, retrying project creation with the same principal, route and key but a different project name returns 409, not a second project. A contributor issuing credentials is denied; a project owner promoting another owner is denied under the selected default, even though ordinary membership edits are permitted. Authenticated account policy governs project creation independently of roles on existing projects. Self-revocation only reduces authority and cannot issue credentials.
...
pointer under a uniqueness constraint; the same key with different content returns a conflict. A repeated exact
...
reconciled, not treated as a usable credential. Reset issuance uses the same metadata-only uncertain-retry convention: the superuser reissues a reset with a new key, atomically invalidating the previous pending reset before returning a new value once. An exact successful redemption retry reconciles the recorded result without changing the password again; a consumed token cannot authorize new content or another operation. Revoke has its own idempotency
