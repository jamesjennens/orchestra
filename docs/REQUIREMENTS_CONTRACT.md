...
Native Beads exports currently carry description, labels and comments; they do not supply this structured revision contract automatically. The snapshot is an explicit bootstrap adapter output. This slice must not pretend to round-trip arbitrary native exports or derive revision/acceptance fields by guessing prose. A native export adapter is a later task.
