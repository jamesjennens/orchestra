...
Operational implementation is now available: evidence-scoped lifecycle events and CURRENT/COORDINATION tables, native events plus durable activity cursors, recoverable native child creation, project merge slots, local transport/CMD/POSIX wrappers, initial backup plus journal recovery, and native decision validation. See OPERATIONAL_WORKFLOW.md and reports/OPERATIONS_VALIDATION.md. Use the typed lifecycle/coordination commands; unrestricted native set-state/merge-slot are not exposed. CURRENT and lifecycle list evaluate scope and evidence, while raw native labels alone may retain a superseded scope. The exact native decision heading is Alternatives Considered. Actual workplace Copilot/separate-account rollout and live-project migration remain outside the synthetic trial.
